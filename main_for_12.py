...
def train(CAEM_with_SNR, fms, alice_verifier, epoch, args, net, alice_bob_mac, key_ab, eve, Alice_KB, Bob_KB, Eve_KB, Alice_mapping, Bob_mapping, Eve_mapping, mi_net=None):  # 当前训练的轮数，命令行参数，模型，互信息网络（默认是None，也就是不训互信息网络）
    ...
    for sents in pbar:  # 每个batch的数据
        ...
        if mi_net is not None:
            ...
            loss_alice, loss_eve, loss_m = train_step(CAEM_with_SNR, fms, alice_verifier,
    ...
    return total_alice / len(train_iterator), total_eve / len(train_iterator), total_m / len(train_iterator)
...
if __name__ == '__main__':
    ...
    os.mkdir("./checkpoints/12/" + now)
    writer = SummaryWriter(log_dir="./logs/12/" + now)
