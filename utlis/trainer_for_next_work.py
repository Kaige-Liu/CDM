...
def train_step(CAEM_with_SNR, fms, alice_verifier, args, epoch, batch, model, alice_bob_mac, key_ab, eve, Alice_KB, Bob_KB, Eve_KB, Alice_mapping, Bob_mapping, Eve_mapping, src, trg, src_eve, n_var, pad, opt_joint, channel, mi_net=None):  # 模型，发送的128个句子，发送的128个句子，噪声标准差(类型数字)，数字0，deepsc优化器，信道类型
    ...
    channels = Channels()
    bs = args.batch_size
    snr_min, snr_max = 3.0, 10.0  # 学习的信噪比区间 不用转换成线性的 线性的反而不好学 因为跨度太大
    ...
    loss = loss_alice + loss_eve

    opt_joint.zero_grad()
    loss.backward()
    opt_joint.step()

    return loss_alice.item(), loss_eve.item()
